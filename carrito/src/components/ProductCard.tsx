...
function ProductCard({ addToCart, product, setAgregads} : ProductCardProp) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-md hover:shadow-lg transition transform hover:scale-[1.02] p-6 flex flex-col items-center text-center">
      {/* Imagen placeholder */}
      ...
      {/* Nombre */}
      <h1 className="text-lg font-bold text-gray-800 mb-1">{name}</h1>
      ...
      > 
        ...
        {setAgregads.includes(product.code) ? 'Agregado '  : 'Comprar'}
      </button>
    </div>
  )
}
